...
- [quizzes/quizzes.json](quizzes/quizzes.json) — the registry of quizzes. Each entry: `id`, `title`, `file` (path to the quiz's JSON), `description`, `subtitle` (shown as a badge on the index card and as the subtitle on the quiz start screen), optional `year`/`quarter` (numbers — sort order within a group on the index page), optional `group` (index-page section heading; entries with no `group` fall under "Examens oefenen"). `loadQuizList()` also fetches each quiz file to show a small meta line (week count, question count, whether an `examen` category exists). **Adding a new quiz means adding an entry here plus creating the quiz JSON file** — see README.md for the (Dutch) contributor-facing version of this.
